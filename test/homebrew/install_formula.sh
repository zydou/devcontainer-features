...
su "$NON_ROOT_USER" -c "/home/${NON_ROOT_USER}/.linuxbrew/bin/diff-so-fancy --colors"
ret=$?
if [ "$ret" -eq '0' ]; then
    echo '✅ Passed(homebrew): diff-so-fancy installed successfully'
    exit 0
else
    echo '❌ Failed(homebrew): diff-so-fancy not found!'
    exit 1
fi